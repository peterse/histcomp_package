#v7 Setting up for an arbitrary environment - added 'path' parameter to Tools
...
class fileTools():
	"""Methods associated with constructing dictionaries in parallel to a ROOT file and analyzing them. Operates in the same context as the rootfile being analyzed; refers to external rfiles at rfile_path"""

	def __init__(self, file_name, rfile_path,  get_leafs=False, fast_channel_count=True, debug=False):
		self.tiers = 0
		self.file_name = str(file_name)
		self.problems = []
		self.status_good = self.check_path(rfile_path)
		#Private general stuff

		# # # # # # # # DIRECTORY AND FILE PATHS # # # # # # # # # # 
		#None: Operates in whatever context it was called in 
		...
		if not self.status_good:
			print "Error executing fileTools.py: %s" % ", ".join(self.problems)
			sys.exit()
